...
def test_dimension_en_cero_es_dato_insuficiente_no_geometria_valida():
    """[fix] Una fila de UMA con Largo/Ancho/Alto = 0.0 (dato REAL, no
    vacío/NaN) es tan inútil como que falte -antes solo se chequeaba
    `is None`, así que una caja de 0x0x0 pasaba de largo hasta el final de
    la cascada de reconciliación con requiere_revision=False, y la SKU
    desaparecía del plan sin ningún aviso (bug real, 18 SKUs / ~295 cajas
    en un dataset de producción)."""
    fila = _row(largo=0.0, ancho=0.0, alto=0.0, cajas_por_cama=10)
    g = reconciliar_sku(fila)
    assert g.fuente_geometria == "DATO_INSUFICIENTE"
    assert g.requiere_revision is True
    assert g.largo_efectivo is None
    assert g.ancho_efectivo is None


def test_alto_en_cero_con_largo_ancho_validos_tambien_es_insuficiente():
    fila = _row(largo=30.0, ancho=20.0, alto=0.0, cajas_por_cama=10)
    g = reconciliar_sku(fila)
    assert g.fuente_geometria == "DATO_INSUFICIENTE"
    assert g.requiere_revision is True
